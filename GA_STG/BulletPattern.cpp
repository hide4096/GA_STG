...
void BulletPattern7(int n) {
	if (shotdata[n].count % 30 == 0) {
		for (int i = 0; i < 6; i++) {
			...
			if (j != -1) {
				shotdata[n].bullet[j].flg = true;
				shotdata[n].bullet[j].count = 0;
				shotdata[n].bullet[j].save = 100;
				shotdata[n].bullet[j].x = chara.x + cos(PI / 3 * i) * 100;
				shotdata[n].bullet[j].y = chara.y + sin(PI / 3 * i) * 100;
				double face2chara = atan2(chara.y - shotdata[n].bullet[j].y, chara.x - shotdata[n].bullet[j].x);
				shotdata[n].bullet[j].angle = face2chara + (PI / 10);
				shotdata[n].bullet[j].speed = 2.0;
				shotdata[n].bullet[j].range = 1.5;
				shotdata[n].bullet[j].img = 0;
			}
		}
	}
	for (int i = 0; i < MAX_BULLET; i++) {
		if (shotdata[n].bullet[i].flg) {
			if (shotdata[n].bullet[i].count > 30 && shotdata[n].bullet[i].count < 90) {
				shotdata[n].bullet[i].angle += PI / 20;
				shotdata[n].bullet[i].speed += 0.1;
			}
		}
	}
}
